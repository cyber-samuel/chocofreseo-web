            ...
            {pedido.productos.map((p, i) => (
              <div key={i} className="revision-producto-item">
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
                  {p.chocolate && (
                    <span style={{ background: p.chocolate==='Negro' ? '#4b5563' : '#f5f0e8', color: p.chocolate==='Negro' ? '#fff' : '#78716c', fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 600 }}>Chocolate {p.chocolate}</span>
                  )}
                  {p.toppings.map((t, ti) => (
