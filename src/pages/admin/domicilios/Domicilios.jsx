...
// Aplana una venta de la API a la forma que espera el render de esta página
    ...
    <div className="modal-overlay">
      <div className="modal-caja modal-pequeno">
        ...
        <p className="modal-texto-confirmar">
          ¿Rechazar el pedido <strong>#V-{String(pedido.id_venta).padStart(4,'0')}</strong>?
        </p>
        <textarea
        ...
        <div className="modal-pie centrado" style={{ marginTop: 16 }}>
          ...
          <button
            ...
            onClick={() => { if (motivo.trim()) { onConfirmar(motivo); setMotivo(''); } }}
      ...
      <div className="modal-overlay">
        <div className="modal-caja modal-pequeno">
          ...
          <div className="modal-pie centrado" style={{ marginTop: 16 }}>
            ...
            <button
              className="btn-peligro"
              onClick={() => { if (motivoRechazo.trim()) { onRechazar(motivoRechazo); reset(); } }}
          ...
          {pedido.productos.map((p, i) => (
            <div key={i} className="revision-producto-item">
              <div className="revision-producto-header">
                ...
                <span className="revision-producto-precio">${p.subtotal.toLocaleString()}</span>
              </div>
              {p.toppings.length > 0 && (
                <div className="revision-extras">
                  ...
                  <div className="extras-chips">
                    {p.toppings.map((t) => (
                      <span key={t} className="chip activo">{t}</span>
                    ))}
                  </div>
                </div>
              )}
              {p.adiciones.length > 0 && (
                <div className="revision-extras">
                  ...
                  <div className="extras-chips">
                    {p.adiciones.map((a) => (
                      <span key={a} className="chip activo">{a}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div key={d.id_venta} className="domi-card">
                ...
                <div className="domi-card-footer">
                  ...
                  <div className="domi-card-acciones">
                    ...
                    {/* WhatsApp — link real con logo SVG */}
                    <a
                      href={urlWpp(d.telefono, d.id_venta)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="btn-accion btn-wpp"
                    ...
                    {/* Rechazar */}
                    <button
                      className="btn-accion btn-rechazar-rapido"
                      onClick={() => setRechazandoRapido(d)}
                    ...
                    {/* Confirmar */}
                    <button
                      className="btn-accion btn-confirmar-rapido"
                      onClick={() => confirmar(d)}
